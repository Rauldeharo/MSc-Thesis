# AUC computation from Fingerprint plots
Plots obtained from this Fingerprint tool, showing the cumulative sum of reads for each replicate in each histone and input, were analyzed computing the AUC value.

Subsequent 2sample t-test comparisons were performed between each histone mark and its input. 

Lower AUC values and greater differences between histone mark and input values represent high confidence sequencing files.
