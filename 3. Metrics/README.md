# Metrics computation between peak-datasets

Peak-datasets generated by both peak-callings where then analyzed in order to extract metrics values on the peaks profile.

The computed metrics were the 1) Coverage Intersection, 2) Jaccard Index and 3) Overlap Coefficient.

Therefore, GenomicRanges (v1.52) (https://bioconductor.org/packages/release/bioc/html/GenomicRanges.html) package from Bioconductor (v3.17) software was implemented in R (v4.3) to transform each BED file obtained from peak-calling into a GRranges object.

Two-by-two comparisons were then performed on different peak-datasets.
